    ...
    TestBenchTop(sc_module_name nm,  uint32_t blockSize, uint32_t cwSize, \
		...
		, mQueueDepth(cmdQueueSize)
        {
			...
			* (uint64_t)(pageSize)* (uint64_t)(pageNum)* (uint64_t)(chanNum));
			...
			mPayloadDataPtr = new uint8_t[BUS_WIDTH];
		...
		for (uint32_t queueIndex = 0; queueIndex < mCmdQueueSize; queueIndex++)
		{
			...
			if (numSlot < mSlotNum)
			{
				...
				if (cmd == WRITE)
				{
					...
					REPORT_INFO(testFile, __FUNCTION__, msg.str());
					for (int i = 0; i < mBlockSize / BUS_WIDTH; i++){
						memcpy(mPayloadDataPtr, mData + i*BUS_WIDTH, BUS_WIDTH);
						sendData(ddrAddress, mPayloadDataPtr, BUS_WIDTH);
						ddrAddress += BUS_WIDTH;
					}
					mSlotManagerObj.insertCmd(cmdPayload, numSlot);
					ddrAddress = getDDRAddress(numSlot, PCMDQUEUE, BL4);
					sendCommand(ddrAddress, (uint8_t*)&cmdPayload, COMMAND_PAYLOAD_SIZE);
	 ...
	 const uint32_t& size)
{
	...
	//sc_core::sc_time delay = m_qk.get_local_time() + duration;
	 ...
	 mDataTransferTime = (double)(BUS_WIDTH * 1000) / (double)(2*BUS_WIDTH * mDdrSpeed);
...
void TestBenchTop::decodeCompletionQueueData(const uint8_t& queueCount, uint8_t& validCount)
{
	...
	for (uint8_t byteIndex = 0; byteIndex < validCount * 3; byteIndex += 3)
	{
		...
		msg << "getCompletionQueue: "
			...
			<< " CwCnt expected: " << dec << (uint32_t)(mSlotTableEntry & pldMask)
			<< " Cmd Type recieved: " << hex << (uint32_t)rxCmdType
			//<< " Cmd Type expected: " << hex << (uint32_t)((mSlotTableEntry >> 46) & 0x3)		
			<< " Cmd Type expected: " << hex << (uint32_t)((mSlotTableEntry >> (mLBAMaskBits + 9)) & 0x3)
		...
		assert(rxCwCount == (mSlotTableEntry & pldMask));
...
uint32_t TestBenchTop::getDDRAddress(const uint8_t& slotNo, const uint8_t& cs, bool CASBL){
	uint32_t ddrAddress = 0;
	if (cs == PCMDQUEUE){
		ddrAddress = (((uint32_t)slotNo << 3) & 0x3ffff) | ((uint32_t)cs << 18);
	}
	else if (cs == DATABUFFER0){
		ddrAddress = (((uint32_t)slotNo << (uint32_t)log2(double(mBlockSize))) & 0x3ffff) | ((uint32_t)cs << 18);
	}
	else if (cs == COMPLETIONQUEUE){
		ddrAddress = (((uint32_t)slotNo) & 0x3ffff) | ((uint32_t)cs << 18);
	}
	return ddrAddress;
}
...
void TestBenchTop::checkData(const uint8_t& slotIndex, const uint16_t& rxCwCount, const uint64_t& lba){
	...
	for (int i = 0; i < mBlockSize / BUS_WIDTH; i++){
		//memcpy(mPayloadDataPtr, mData + i*BUS_WIDTH, BUS_WIDTH);
		loadData(ddrAddress, mPayloadDataPtr, BUS_WIDTH);
		memcpy(rxData + i*BUS_WIDTH, mPayloadDataPtr, BUS_WIDTH);
		ddrAddress += BUS_WIDTH;
	}
...
#if 0
		assert((uint32_t)rxData[i] == (uint32_t)expData[i]);
#else
		std::cout << "Rx Data: " << hex << (uint32_t)rxData[i]
