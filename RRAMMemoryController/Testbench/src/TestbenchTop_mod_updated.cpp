		...
		while (slotReqIndex < (int16_t)numSlotReq)
		{
			if (mSlotManagerObj.getAvailableSlot(numSlot))
			{
				...
				if (!firstSlotIndexed)
				{
					...
					firstSlotIndexed = true;
					mSlotManagerObj.addFirstSlotNum(tags, numSlot);
					mSubQueue.front().slotNum = numSlot;
					//mLatency.at(tags).startDelay = sc_time_stamp().to_double();
				}
				mSlotManagerObj.addSlotToList(tags, numSlot);
				mSlotContainer.push_back(numSlot);
			...
			else
			{
				...
				processCompletionQueue(queueEntry);
			}
		}//while(slotReqIndex < (int16_t)numSlotReq)
		firstSlotIndexed = false;
		lba = 0;
...
void TestBenchTop::findQueueEntry(uint32_t &qIndex, const uint32_t tags){
	std::ostringstream msg;
	uint32_t index;
	index = find(mSubQueue.begin(), mSubQueue.end(), findSlot(tags)) - mSubQueue.begin();
	if (index < mSubQueue.size()){
		qIndex = index;
	}
	else{
		msg.str(" ");
		msg << "findQueueEntry ::tag not found" << endl;
		REPORT_ERROR(testFile, __FUNCTION__, msg.str());
	}
}
...
void TestBenchTop::freeSlot(uint16_t& slotIndex)
{
	...
	if (mSlotManagerObj.removeSlotFromList(tags, slotIndex))
	{
		...
		{
			uint32_t qIndex = 0;
			findQueueEntry(qIndex, tags);
			queueEntry = mSubQueue.at(qIndex);

			latency = sc_time_stamp().to_double - queueEntry.mLatency.startDelay;
			//queueEntry.mLatency.at(qIndex).endDelay = sc_time_stamp().to_double();
			//queueEntry.mLatency.at(tags).latency = mLatency.at(tags).endDelay - mLatency.at(tags).startDelay;
			queueEntry.mLatency.latency = latency;

			//free Submission queue
			mSubQueue.erase(mSubQueue.begin() + (qIndex - 1));

			mSlotManagerObj.resetTags(tags);
			uint16_t slotNum = mSlotManagerObj.getFirstSlotNum(tags);
			mLatencyRep->writeFile(tags, queueEntry.mLatency.latency / 1000, " ");
			mLatencyRepCsv->writeFile(tags, queueEntry.mLatency.latency / 1000, ",");
			//cmdCompleteCount++;
			mIsCmdCompleted = true;

		}
		mTrigSubCmdEvent.notify();
		mSlotManagerObj.freeSlot(slotIndex);
	}//if
}
...
void TestBenchTop::submissionQueueThread()
{
	...
	while (1)
	{
		uint32_t freeSpace = mQueueDepth;
		if (cmdIndex < mNumCmd)
		{
				for (int queueIndex = 0; queueIndex < freeSpace; queueIndex++)
			{
				mTrafficGen.getCommands(payload);
				CmdQueueField queueEntry;
				
				queueEntry.mLatency.startDelay = sc_time_stamp().to_double();
				queueEntry.mLatency.endDelay = 0;
				queueEntry.mLatency.latency = 0;
				...
				if (mSubQueue.size() == mQueueDepth)
				{
					...
					freeSpace = mQueueDepth - mSubQueue.size();
				}
			}
...
bool TestBenchTop::pollStatusWaitTime(bool pollFlag)
{
	if (pollFlag)
	{
		wait(mPollWaitTime, SC_NS);
...
void TestBenchTop::setCmdParameter(uint32_t ioSize, uint32_t& cwCount, uint32_t& lastCount, uint8_t& numSlotReq)
{
	uint8_t remSlot;
	uint32_t tempCwCnt;
	/*Number of slots required */
...
void TestBenchTop::pushSlotToList(uint16_t tags, uint16_t numSlot)
{
	mSlotManagerObj.getFreeTags(tags);
	mSlotManagerObj.addFirstSlotNum(tags, numSlot);
	mLatency.at(tags).startDelay = sc_time_stamp().to_double();
...
bool TestBenchTop::getCmdTypeFromCmplQueue(uint8_t& cmdIndex)
{
	return ((bool)((*((uint16_t*)mReadCompletionQueue + cmdIndex * COMPLETION_WORD_SIZE + 1) >> 15) & 0x1));
}

uint8_t TestBenchTop::getSlotFromCmplQueue(uint8_t& cmdIndex)
{
	uint8_t slotNum;
...
void TestBenchTop::popSlotFromQueue(uint16_t slotIndex)
{
