...
public:
	...
	void update(const uint32_t& cmd, const uint32_t& lbaMaskBits);
	uint16_t getCount();
	uint8_t getSlotNum();
...
private:
	...
	void getSlotAndCwCnt(const uint32_t cmd, uint16_t& slotNum, uint16_t& cwCount, const uint32_t& lbaMaskBits);
